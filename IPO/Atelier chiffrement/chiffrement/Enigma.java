...
public class Enigma
{
    ...
    public static void main(final String pArgs[])
    {
        ...
        int[] vPositionsInitiales = strToPos(vMessage);
        int[] vPositionsChiffrees = new int[vPositionsInitiales.length];
        for (int i = 0; i < vPositionsInitiales.length; i++) {
            vPositionsChiffrees[i] = vRotor.chiffreLettre(vPositionsInitiales[i]);
        }
        String vResultat = posToStr(vPositionsChiffrees);
        System.out.println("Message original : " + vMessage);
        System.out.println("Message chiffré  : " + vResultat);
...
class Rotor  // classe non accessible en dehors de ce fichier
{
    ...
    public void decalRotor(final int pDirection, final int pRotorActif)
    {
        ...
        public int chiffreLettre(int pPosLettre) {
        int vPos = pPosLettre;

        vPos = (26 + vPos + aRotor1[1][vPos]) % 26; 
        vPos = (26 + vPos + aRotor2[1][vPos]) % 26; 
        vPos = (26 + vPos + aRotor3[1][vPos]) % 26; 
        vPos = (vPos + aReflecteur[vPos] + 26) % 26;
        vPos = (26 + vPos + aRotor3[0][vPos]) % 26;
        vPos = (26 + vPos + aRotor2[0][vPos] ) % 26;
        vPos = (26 + vPos + aRotor1[0][vPos]) % 26;
