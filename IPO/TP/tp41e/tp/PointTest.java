...
public class PointTest
{
    ...
    public void test3_constructeurs()
    {
        ...
        test_params( 2, new String[]{"int","int"} );
    ...
    public void test4_deplace()
    {
        ...
        try {
            int vX=12, vY=24;
            int vDX=5, vDY=-5;
            String vSNX=""+(vX+vDX), vSNY=""+(vY+vDY);
            vIns = sCon.newInstance( new Object[]{vX,vY} );
            try {
                sMetName = "deplace";
                sMet.invoke( vIns, new Object[]{vDX,vDY} );
                sAttType = "int";
                sAttName = "aX";
                veref.FieldContent vAtt1 = veref.V.selAttFTN( sCla, sAttType, sAttName );
                veref.V.vrai( vAtt1.fieldValue( vIns ).equals(vSNX),
                    sAttName+" incorrect ("+vAtt1.fieldValue( vIns )+"/"+vSNX+") apres deplace !" );
                veref.V.failIfNot();        
                sAttName = "aY";
                veref.FieldContent vAtt2 = veref.V.selAttFTN( sCla, sAttType, sAttName );
                veref.V.vrai( vAtt2.fieldValue( vIns ).equals(vSNY),
                    sAttName+" incorrect ("+vAtt2.fieldValue( vIns )+"/"+vSNY+") apres deplace !" );
                veref.V.failIfNot();        
    ...
    public void test6_affiche()
    {
        ...
        veref.V.verifFileContains( sFil, "println\\s*\\(.*\"<<.*\\)", "l'affichage de <<", true, false );
    ...
    public void test7_equals()
    {
        ...
        sMet = veref.V.getVMetFProto( sCla, sMetName, sMetRT, sProtoM );
        veref.V.mesIfNot();        
        test_params( 1, new String[]{"Object"} );
        veref.V.verifMetOverDebug( sFil, "public", sMetRT, sMetName, false );
    } // test7_equals()

    private void test_params( final int pNPar, final String[] pTypes )
    {
        for ( int vI=pNPar; vI<=pNPar; vI++ ) {
            String vRet = veref.V.verifFinalN( sFil, sMetName, vI );
            veref.V.vrai( vRet.equals( "OK" ), vRet );
            veref.V.failIfNot();        
            veref.V.verifParamPNType( sFil, sMetName, pTypes, vI );
        } // for
    } // test_params(.)
} // PointTest
