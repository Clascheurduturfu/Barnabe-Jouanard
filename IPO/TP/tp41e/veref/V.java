...
public class V
{
    ...
    private static PrintStream sConsole /*= null*/;
    ...
    public static int nbAtt( ClassContent pCla )
    {
        int vNbAtt = pCla.getFields().getNbFields();
        return vNbAtt;
    } // nbAtt(...)
    ...
    {
        // verifDefCon( pCla, "F" ); // le DefCon peut aussi exister !
    ...
    @Test
    public static Object getAndVerifIns2( final ConstructorContent pC, final FieldContent pF1,
    final Object pV1, final FieldContent pF2, final Object pV2 )
    {
        Object vIns = null;
        try {
        ...
        vrai( pV1.equals( pF1.fieldValue( vIns ) ),
            "Le constructeur n'initialise pas correctement l'attribut" + pF1
            + " (" + pF1.fieldValue( vIns ) + "?)" );
        failIfNot();
        vrai( pV2.equals( pF2.fieldValue( vIns ) ),
            "Le constructeur n'initialise pas correctement l'attribut" + pF2
            + " (" + pF2.fieldValue( vIns ) + "?)" );
        failIfNot();
        return vIns;
    } // getAndVerifIns2(.....)
    ...
    @Test
    public static Object getAndVerifInsDef2( final ConstructorContent pC, final FieldContent pF1, final Object pV1,
    final FieldContent pF2, final Object pV2 )
    {
        Object vIns = null;
        try {
            vIns = pC.newInstance( new Object[]{} );
        } catch( final Exception pE ) {
            fail( "le constructeur " + pC.getName() + pC.getParameterString() + " genere une exception !" );
        } // t/c
        vrai( pV1.equals( pF1.fieldValue( vIns ) ),
    ...
    // getMetFProto et MethodsContent ne savent pas recuperer les methode statiques !!!
    ...
    {
        return getVMetFProto( pCla, pMethod, pRT, pProto, "public", "static final" );
    } // getVMetFProto(....)
    ...
    @Test
    public static MethodContent getVMetFProto( final ClassContent pCla, final String pMethod, final String pRT,
    final String pProto, final String pModY, final String pModN )
    {
        verifMet( pCla, pMethod, "T" );
        verifMetRT( pCla, pMethod, pRT, "T" );
        verifMetRTNP( pCla, pMethod, pRT, calcNPFProto(pProto), "T" );
        MethodContent vMet = getMetFProto( pCla, pMethod, pRT, pProto, "T" );
        verifModMet( vMet, pModY, pModN );
        return vMet;
    } // getVMetFProto(......)
    ...
    final String pRT, final int pNP, final String pExist )
    {
        ...
        String vMessage = ( vWanted ? "n'existe pas" : "ne devrait pas exister" );
        MethodsContent vLMet = new MethodsContent( pCla.className() );
        vrai( vLMet.hasMethodRTNP( pMethod, pRT, pNP ) == vWanted, "$a avec $n parametres(s) " + vMessage + " !" );
        failIfNot();
    } // verifMetRTNP(.....)
    ...
    public static boolean redirectTerminal()
    {
        ...
        try {
            sConsole = System.out;
            sOut = new PrintStream( new BufferedOutputStream(
    ...
    public String compareTerminal( String refName )
    {
        try {
            ...
            String liRef, liRes="beginning";
            while ( ref.hasNextLine() ) {
                liRef = ref.nextLine();
                if ( ! res.hasNextLine() )
                    return "Il manque au moins une ligne d'affichage après " + liRes + " !" ;
                liRes = res.nextLine();
        ...
        finally { System.setOut( sConsole ); }
        return "OK";
    } // compareTerminal(.)
    ...
    public static void verifFinal1Type( String pFil, String pMet, String pType ) // un seul parametre
    {
        ...
        try {
            while ( vJ.hasNextLine() ) {
                ...
                if ( vL.matches( vSans ) ) {
                    sRetour = null;
                    failIfNot( "le parametre $a $n($t) n'est pas final (vérifiez les suivants !)" );
                } // if bad
            } // while
        } catch( Exception pE ) {
    ...
    public static void verifParamPNType( String pFil, String pMet, String[] pTypes, int pN ) // Neme parametre
    {
        boolean pDebug = false; // devrait être un vrai paramètre !
        if ( pN == 0 )  return;
        ...
        if ( pN == 1 ) {
            verifParamP1Type( pFil, pMet, pTypes[0] );
            return;
        }
        ...
        for ( int vI=0; vI<pN; vI++ ) {
            sType = pTypes[vI];
            vP = "p";
            vParamA = "\\s*"+"final\\s+"+sType+"\\s+"+vP+"\\w+\\s*";
            vP = "";
            vParamS = "\\s*"+"final\\s+"+sType+"\\s+"+vP+"\\w+\\s*";
            vAvec += vParamA;
            vSans += vParamS;
            if ( vI < pN-1 ) {
                vAvec += "\\,";
        ...
        try {
            while ( vJ.hasNextLine() ) {
                vL = vJ.nextLine();
                if ( pDebug )
                    System.out.println( "verifParamPNType"+":"+vL.matches(vAvec)+vL.matches(vSans)+vL );
                if ( vL.matches( vAvec ) )
                    return;
                if ( vL.matches( vSans ) ) {
                    sRetour = null;
                    failIfNot( "le nom du parametre "+pN+" $a $n ne commence pas par 'p' !" );
                } // if bad
            } // while
        } catch( Exception pE ) {
            error( "code source de "+pFil+" non analysable : " + pE );
    ...
    { 
        ...
        final String vSauf3 = ".*"+"\\w"+pAtt+".*"; // pour alphanum juste avant nom (pDeltaX pour aX)
        final String vCom1 = ".*/\\*\\*.*\\*/.*"; // commentaire javadoc sur une seule ligne
        final String vCom2 = ".*//.*"; // commentaire mono-ligne
        String vL;
        int vCpt = 0;
        ...
        try {
            while ( vJ.hasNextLine() ) {
                vCpt++;
                vL = vJ.nextLine();
                if ( vL.matches( vCom1 ) || vL.matches( vCom2 ) )
                    continue;
                if ( vL.matches( "\\s*/\\*\\*.*" ) ) {
                    vInCom = true;
                    continue;
                ...
                if ( pDebug )
                    System.out.println( ""+vInCom+":"+vL.matches(vAvec)+vL.matches(vSans)
                        +vL.matches(vSauf)+vL.matches(vSauf2)+vL.matches(vSauf3)+vL );
                if ( vL.matches( vCom1 ) )
                    continue;
                if ( vL.matches( vAvec ) )
                    continue;
                if ( vL.matches( vSans ) ) {
                    if ( vL.matches( vSauf ) || vL.matches( vSauf2 ) || vL.matches( vSauf3 ) )
                        continue;
                    sRetour = null;
                    if ( ! vInCom )
    ...
    public static void verifMetThis( String pFil, String pType, String pMet )
    { // attention : ne marche pas si methode citee dans commentaires
        verifMetThisDebug( pFil, pType, pMet, false );
    } // verifMetThis(...)

    public static void verifMetThisDebug( String pFil, String pType, String pMet, boolean pDebug )
    { // attention : ne marche pas si methode citee dans commentaires
        sType   = pType;
        sNom    = pMet;
        sNature = "La methode";
        ...
        // Pourquoi ne peut-on pas ajouter \\)  avant .*  ?
        final String vAvec = ".*"+"this\\."+pMet+"\\s*\\(.*";
        final String vSans = ".*"          +pMet+"\\s*\\(.*";
        final String vSauf = ".*"+pType+"\\s+"+pMet+".*";
        final String vSauf2 = "\\s*"+"}\\s*//\\s*"+pMet+".*"; // pour accolade fin
        ...
        try {
            while ( vJ.hasNextLine() ) {
                ...
                if ( vL.matches( vSans ) ) {
                    ...
                    sRetour = null;
                    if ( ! vInCom )
                        failIfNot( "$a $n n'est pas precedee de this. a la ligne " + vCpt );
                } // if bad
            } // while
        } catch( Exception pE ) {
            error( "code source de "+pFil+" non analysable : " + pE );
    ...
    public static void verifConThisParDebug( String pFil, int pNb, boolean pDebug ) // un seul parametre
    { 
        sNature = "Le constructeur";
        Scanner vJ = null;
        try {
        ...
        // Pourquoi ne peut-on pas ajouter \\)  avant .*  ?
        final String vAvec = ".*"    +"this\\s*\\("+".*";
        final String vSauf = ".*//.*"+"this\\s*\\("+".*";
        String vL;
        int vCpt = 0;
        int vNbThis = 0;
        try {
            while ( vJ.hasNextLine() ) {
                ...
                vL = vJ.nextLine();
                if ( pDebug )
                    System.out.println( ""+vL.matches(vAvec)+vL.matches(vSauf)+vL );
                if ( vL.matches( vAvec ) && ! vL.matches( vSauf ) ) {
                    vNbThis++;
                } // if
            } // while
            sRetour = null;
            if ( vNbThis != pNb )
                failIfNot( "Un constructeur n'appelle pas l'autre a chaque fois que c'est possible "
                    + "ou il reste de la duplication de code !?" );
        } catch( Exception pE ) {
            error( "code source de "+pFil+" non analysable : " + pE );
        } // t/c
    } // verifConThisParDebug(....)

    public static void verifMetOverDebug( String pFil, String pProt, String pType, String pMet, boolean pDebug ) // un seul parametre
    { 
        sType   = pType;
        sNom    = pMet;
        sNature = "La methode";
        ...
        // Pourquoi ne peut-on pas ajouter \\)  avant .*  ?
        final String vOver = "\\s*"+"\\@Override"+"\\s*";
        final String vAvec = ".*"+"\\@Override"+"\\s+"+pProt+"\\s+"+pType+"\\s+"+pMet+"\\s*\\(.*";
        final String vSans = ".*"+              "\\s+"+pProt+"\\s+"+pType+"\\s+"+pMet+"\\s*\\(.*";
        final String vSauf = ".*"+"//"+vAvec;
        // final String vSauf2 = ".*"+"et"+pAtt+".*"; // pour get et set
        final String vSauf2 = ".*"+"\\w+"+"rien"+".*";
        String vL = "rien";
        String vPrec;
        int vCpt = 0;
        try { // a ameliorer pour accepter @Override a la ligne precedente
            while ( vJ.hasNextLine() ) {
                vCpt++;
                vPrec = vL;
                vL = vJ.nextLine();
                ...
                if ( vPrec.matches(vOver) && vL.matches( vSans ) && ! vL.matches( vSauf ) )
                    return;
                if ( vL.matches( vAvec ) && ! vL.matches( vSauf ) )
                    return;
                if ( vL.matches( vSans ) && ! vL.matches( vSauf )) {
                    sRetour = null;
                    failIfNot( "$a $n n'est pas precedee de @Override a la ligne " + vCpt );
                    return;
                } // if bad
            } // while
        } catch( Exception pE ) {
    ...
    public static void verifFileContains( String pFil, String pPat, String pMes, boolean pYes, boolean pDebug )
    { 
        sNom    = pFil;
        sNature = "La classe";
        Scanner vJ = null;
        ...
        try {
            while ( vJ.hasNextLine() ) {
                ...
                vL = vJ.nextLine();
                if ( pDebug )
                    System.out.println( ""+vL.matches(vAvec)+":"+vL );
                if ( vFound = vL.matches( vAvec ) )
                    if (pYes)
                        return; // on a trouve ce qu'on voulait
                    else
                        break; // on a trouve ce qu'on ne voulait pas
            } // while
            sRetour = null;
            if (pYes)
                failIfNot( "A la ligne "+vCpt+", $a $n ne contient pas " + pMes + " ???" );
            if (vFound)
                failIfNot( "A la ligne "+vCpt+", $a $n contient " + pMes + " ???" );
        } catch( Exception pE ) {
            error( "code source de "+pFil+" non analysable : " + pE );
        } // t/c
    } // verifFileContains(....)
    ...
    public static void verifCount( String pFil, String pTarget, int pN ) // un seul parametre
    {
        verifCountDebug( pFil, pTarget, pN, false );
    } // verifMetThis(...)

    public static void verifCountDebug( String pFil, String pTarget, int pN, boolean pDebug ) // un seul parametre
    {
        sNature = "La methode";
        Scanner vJ = null;
        try {
        ...
        final String vAvec = ".*"+pTarget+".*";
        //        final String vSans = ".*"   +"\\s+"+pMet+"\\(.*";
        final String vSauf = ".*//"+pTarget+".*";
        String vL;
        int vCpt = 0;
        int vCount = 0;
        try {
            ...
            if ( vCount < pN ) {
                sRetour = null;
                failIfNot( "Vous n'utilisez pas suffisamment " + pTarget + " : " + vCount + "/" + pN );
            } // if not enough
            else if ( pN<0 && vCount>0 ) {
                sRetour = null;
                failIfNot( "Vous ne devez pas utiliser " + pTarget + " : " + vCount + "/0" );
            } // if not enough
        } catch( Exception pE ) {
            error( "code source de "+pFil+" non analysable : " + pE );
        } // t/c
    ...
    public static void verifThisPar( String pFil, int pNbAtt ) // un seul parametre
    { // attention : ne marche pas si attribut cite dans commentaires
        verifThisParDebug( pFil, pNbAtt, false );
    } // verifThisPar(..)

    public static void verifThisParDebug( String pFil, int pNbAtt, boolean pDebug ) // un seul parametre
    { // attention : ne marche pas si attribut cite dans commentaires
        Scanner vJ = null;
        try {
            vJ = new Scanner( new File( pFil ) );
        ...
        String vParams = "\\s*";
        if ( pNbAtt >= 1 )   vParams += ".*\\w+.*";
        for ( int i=1; i<pNbAtt; i++ )   vParams += ",.*\\w+.*";
        // Pourquoi ne peut-on pas ajouter \\)  avant .*  ?
        final String vAvec = ".*"+"this\\s*\\("+vParams+"\\).*;.*";
        final String vSauf = ".*//"+vAvec;
        String vL;
        int vCpt = 0;
        try {
            ...
            sRetour = null;
            failIfNot( "Un constructeur n'appelle pas l'autre avec "+pNbAtt+" parametres !?" );
        } catch( Exception pE ) {
            error( "code source de "+pFil+" non analysable : " + pE );
        } // t/c
    } // verifThisParDebug(...)

    public static String countWord( String pFil, String pW, int pN ) // presence mot
    {
        //        System.out.println(System.getProperty("user.dir"));
        Scanner vJ = null;
        try {
        ...
        try {
            while ( vJ.hasNextLine() ) {
                vL = vJ.nextLine();
                boolean vFound = vL.matches( ".*[^A-Za-z0-9]"+pW+"[^A-Za-z0-9].*" );
                if ( vFound )   vCpt++;
            } // while
        } catch( Exception pE ) {
            return "code source de "+pFil+" non analysable : " + pE;
